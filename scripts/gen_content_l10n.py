#!/usr/bin/env python3
"""Génère Limbator/Services/ContentTranslations.swift.

Le contenu pédagogique de Limbator est écrit en ROUMAIN — la langue de
l'apprenant visé. Cette couche le traduit à la volée quand l'utilisateur choisit
une autre langue d'interface. La table est tenue ici, en un seul endroit, et le
Swift en est dérivé.

Couverture : tout ce qui structure l'écran (noms de modules, de niveaux, de
thèmes, d'histoires, de jeux, d'insignes, catégories de fautes, titres de règles
et moyens mnémotechniques, natures grammaticales). Les textes longs — énoncés de
règles, explications d'exercices, indices de dictée — restent en roumain quand
la langue change : le repli est explicite et sans perte de sens, puisque la
chaîne source s'affiche telle quelle.
"""
...
add({
 ...
 "Un tânăr sculptor din Hobița ajunge la Paris pe jos. Atelierul din Impasse Ronsin, marmura, tăcerea — și primele lui cuvinte în franceză.":
   ("Un jeune sculpteur de Hobița arrive à Paris à pied. L'atelier de l'impasse Ronsin, le marbre, le silence — et ses premiers mots de français.",
    "A young sculptor from Hobița walks to Paris. The studio on impasse Ronsin, the marble, the silence — and his first words of French."),
 "Ioana coboară în Gare du Nord cu două valize și un dicționar. Prima zi, prima cafea, primul formular de completat — corect.":
   ("Ioana descend à la gare du Nord avec deux valises et un dictionnaire. Premier jour, premier café, premier formulaire à remplir — correctement.",
    "Ioana steps off at Gare du Nord with two suitcases and a dictionary. First day, first coffee, first form to fill in — correctly."),
 "Un castel, o fereastră care nu se închide niciodată și un cuvânt cu accent circonflex care ascunde o literă dispărută.":
   ("Un château, une fenêtre qui ne se ferme jamais, et un mot à accent circonflexe qui cache une lettre disparue.",
    "A château, a window that never shuts, and a circumflexed word hiding a vanished letter."),
 "Doi prieteni, o scară de o sută de trepte și o noapte în care fiecare accent contează.":
   ("Deux amis, un escalier de cent marches, et une nuit où chaque accent compte.",
    "Two friends, a hundred-step staircase, and a night where every accent counts."),
...
# ---- Titres de règles et moyens mnémotechniques ----------------------------
add({
...
# ---- Natures grammaticales (familles d'homophones) -------------------------
add({
...
def main():
    ...
    for ro, (fr, en) in T.items():
        lines.append(f'        "{escape(ro)}": ("{escape(fr)}", "{escape(en)}"),')
    lines.append("    ]")
    lines.append("}")
    out = ROOT / "Limbator/Services/ContentTranslations.swift"
    out.write_text("\n".join(lines) + "\n")
    print(f"{out.relative_to(ROOT)} — {len(T)} entrées x 2 langues")
